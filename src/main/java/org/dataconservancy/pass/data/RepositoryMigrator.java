...
public class RepositoryMigrator {
    ...
    static final Logger LOG = LoggerFactory.getLogger(RepositoryMigrator.class);

    private static final String BASE_URL = FedoraConfig.getBaseUrl();

    static void migrateRepositories(PassClient client) {
        LOG.info("Migrating repositories...");
        migrateDEC(client);
        migrateERIC(client);
        migrateJ10P(client);
        migratePMC(client);
    ...
    static void migratePMC(PassClient client) {
        try {
            LOG.info("Migrating PMC");
            final Repository pmc = client.readResource(URI.create(BASE_URL +
                    "repositories/77/64/12/ec/776412ec-0f5e-488e-97dc-15bb427d27e2"), Repository.class);
            pmc.setRepositoryKey("pmc");
            client.updateResource(pmc);
        } catch (final Exception e) {
            LOG.error("Could not updatePMC repository", e.getMessage());
            throw new RuntimeException(e);
        }
    }

    static void migrateJ10P(PassClient client) {
        URI jscholarshipRepoUri;
        try {
            LOG.info("Migrating Jscholarship");
            jscholarshipRepoUri = new URI(BASE_URL + "repositories/41/96/0a/92/41960a92-d3f8-4616-86a6-9e9cadc1a269");
            final Repository repository = client.readResource(jscholarshipRepoUri, Repository.class);

            final String formSchema = "{" + "\"id\":\"JScholarship\"," + "\"schema\":{" +
                    ...
                    "\"type\":\"object\"," + "\"properties\":{" + "\"authors\":{" +
                    ...
                    "\"options\":{" + "\"fields\":{" + "\"authors\":{" + "\"hidden\":false" + "}" + "}" + "}" + "}";

            final String agreementText =
                    "NON-EXCLUSIVE LICENSE FOR USE OF MATERIALS This non-exclusive license defines the terms for the deposit of Materials in all formats " +
            ...
            repository.setRepositoryKey("jscholarship");
    ...
    public static void migrateDEC(PassClient client) {
        final String formSchema = "{" + "\"id\":\"dec\"," + "\"schema\":{" +
                "\"title\":\"United States Agency for International Development - DEC <br><small>Requires deposit to the Development Experience Clearingshouse (DEC).</small><br><br><p class='lead text-muted'>Automatic submission from PASS to ERIC is not available at this time. Instead, submission to DEC will be prompted before this submission is finalized in PASS. The publication information collected so far can be used to complete the submission via the DEC website.</p>\"," +
                "\"type\":\"object\"," + "\"properties\":{}" + "}," + "\"options\":{" + "\"fields\":{}" + "}" + "}";

        try {
            LOG.info("Migrating DEC");
            final Repository dec = client.readResource(URI.create(BASE_URL +
                    "repositories/77/cc/80/64/77cc8064-a918-4823-968d-2b17386db76d"), Repository.class);
            dec.setFormSchema(formSchema);
            dec.setRepositoryKey("dec");
            client.updateResource(dec);
        } catch (final Exception e) {
            LOG.error("Could not update DEC repository", e.getMessage());
            throw new RuntimeException(e);
        }
    }

    public static void migrateERIC(PassClient client) {
        final String formSchema = "{" + "\"id\":\"eric\"," + "\"schema\":{" +
        ...
        try {
            LOG.info("Migrating ERIC");
            final Repository eric = client.readResource(URI.create(BASE_URL +
                    "repositories/9c/10/6a/13/9c106a13-122b-4775-8c04-4f9c926ee451"), Repository.class);
            eric.setFormSchema(formSchema);
            eric.setRepositoryKey("eric");
            client.updateResource(eric);
        } catch (final Exception e) {
            LOG.error("Could not update ERIC repository", e.getMessage());
            throw new RuntimeException(e);
        }
    }

    // For testing, or manually running in eclipse
